...
iconset_root="${stage_root}/shadowsync.iconset"
tar_artifact="${release_dir}/shadowsync-macos-${arch_label}-v${version}.tar.gz"
dmg_artifact="${release_dir}/shadowsync-macos-${arch_label}-v${version}.dmg"

rm -rf "${stage_root}"
mkdir -p "${macos_root}" "${resources_root}" "${iconset_root}"

cp "${binary_path}" "${macos_root}/shadowsync"
chmod +x "${macos_root}/shadowsync"
...
<dict>
  ...
  <string>ShadowSync</string>
  <key>CFBundleExecutable</key>
  <string>shadowsync</string>
  <key>CFBundleIconFile</key>
  <string>shadowsync.icns</string>
  <key>CFBundleIdentifier</key>
  <string>com.rad.shadowsync</string>
...
iconutil -c icns "${iconset_root}" -o "${resources_root}/shadowsync.icns"
...
hdiutil create -volname "ShadowSync" -srcfolder "${dmg_root}" -ov -format UDZO "${dmg_artifact}"
...
if [[ -n "${MACOS_SIGNING_IDENTITY:-}" && -n "${MACOS_NOTARY_APPLE_ID:-}" && -n "${MACOS_NOTARY_TEAM_ID:-}" && -n "${MACOS_NOTARY_APP_PASSWORD:-}" ]]; then
  xcrun notarytool store-credentials "shadowsync-notary" \
